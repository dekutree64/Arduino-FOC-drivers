...
The ADCs are set up in independent mode with 42.5MHz clock and 4x oversampling. There is time for ~128 samples 
on each ADC per iteration of motor.move and loopFOC (50-60µs). Channels can be enabled for regular mode which 
converts continuously into a DMA buffer for increased oversampling (max 9 channels per ADC), or injected mode 
(max 4 channels per ADC) which converts each time startInjectedConversions is called (typically once per loopFOC).
...
The size of the DMA buffer for regular channels is adjusted so it overwrites every ~50µs. When a channel result is 
requested, all its entries in the buffer are averaged together and scaled to 0-16383 range. Compared to higher hardware 
oversampling, this method averages each value over the last 50µs period rather than being random chance whether you 
get the most recently sampled channel or one that's a full 50µs out of date just before its new result comes in.
...
encoder0A. If using regular mode, ensure that both conversion sequences are the same length.
